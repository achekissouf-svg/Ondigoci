                ...
                <div class="card-body p-5">
                    <form action="{{ route('admin.produits.update', $produit->id_produit) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Image du produit (laisser vide pour garder l'actuelle)</label>
                                <input type="file" name="image" class="form-control @error('image') is-invalid @enderror" accept="image/*">
                                @error('image')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                @if($produit->image_principale_produit)
                                    <div class="mt-2">
                                        <img src="{{ asset('images/' . $produit->image_principale_produit) }}" alt="Image actuelle" style="width: 100px; height: 100px; object-fit: contain; border-radius: 8px; border: 1px solid #ddd;">
                                    </div>
                                @endif
                            </div>
                        </div>
